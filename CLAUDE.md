...
- **OAuth token behaviour (empirical, live-tested 2026-07-28):** the token endpoint accepts **HTTP Basic** client auth (`client_id:client_secret`); the authorize endpoint rejects unregistered `redirect_uri` with a plain error page (the URI must be added verbatim in the Fiken app — `http://localhost:8473/callback`, http is allowed for `localhost`). Access tokens live **~1 hour** (`expires_in` ≈ 3600), so refresh matters. On `grant_type=refresh_token` Fiken returns a **new access token but the same refresh token** (not rotated) — `FikenClient._refresh` keeps the old refresh token when the response omits one. **Refresh-token lifetime is not disclosed**: the token response carries no `refresh_token_expires_in` (only the access token's `expires_in`), and Fiken's docs only state the refresh token stays valid until the user revokes app consent (Brukerinnstillinger on fiken.no) — treat it as long-lived-until-revoked; re-run `fiken-auth` if it ever 401s on refresh. No `scope` param is sent (the app registration decides scope).
- **Journal entries are created on a different endpoint than they are read from.** `/journalEntries` is **GET-only** — `POST` returns **405**. Manual entries go to `POST /companies/{slug}/generalJournalEntries` with a nested body: `{description, journalEntries: [{description, date, lines}]}`. Lines do **not** use signed amounts: each line carries a positive `amount` plus `debitAccount` and/or `creditAccount` (`_to_fiken_lines()` translates the tool's signed `{amount, account}` interface — a single debit/credit pair collapses into one line, anything wider becomes separate debit-only/credit-only lines). VAT is `debitVatCode`/`creditVatCode`, not `vatCode`.
- **`description` on a manual entry is capped at 166 characters, not 200.** Fiken prepends `Fri postering registrert via API: ` (34 chars) and then validates the *concatenated* string against its 200-char limit. Over 200 it returns **500** (not 400) with `Transaksjonsbeskrivelse kan ikke være på mer enn 200 tegn`. `MAX_DESCRIPTION` in `tools/journal.py` guards this at dry-run time.
- **A successful entry creation returns `201` with an empty body** (`{}`), not the created object. Verify via `/journalEntries` or an account balance.
